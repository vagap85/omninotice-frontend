...
const featureCards: FeatureCard[] = [
  {
    ...
    description: "Создаем более глубокую связь с аудиторией и помогаем повысить уровень их лояльности",
    icon: cardHeart,
  },
  {
    title: "Рост конверсии на 30-40%",
    description: "Благодаря персонализированным и целенаправленным коммуникациям",
    icon: cardTrending,
  },
  {
  ...
  },
  {
    ...
    description: "Позволяет мгновенно отслеживать эффективность кампаний, адаптируя стратегию",
    icon: cardMonitor,
  },
  {
    title: "Персонализация коммуникаций",
    description: "Увеличиваем вовлеченность клиентов, адаптируя сообщения под их интересы",
    icon: cardUser,
  },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isInsideMobile = mobileMenuContainerRef.current?.contains(target) ?? false;
      const isInsideDesktop = desktopMenuContainerRef.current?.contains(target) ?? false;
  ...
  return (
    <Box minH="100vh" bg="white" color="#12233F">
      <Container maxW="1760px" px={{ base: 4, md: 6, xl: 8 }} py={5} position="relative">
        <Flex
        ...
        <VStack mt={4} spacing={{ base: 3, md: 4 }} align="stretch" display={{ base: "flex", lg: "none" }}>
          <Flex align="center" justify="space-between" gap={3}>
            ...
            <HStack spacing={{ base: 1, md: 5 }}>
              ...
              <Button
                ...
                fontSize={{ base: "12px", md: "14px" }}
                lineHeight={{ base: "16px", md: "20px" }}
                rightIcon={<Image src={arrowRight} alt="" boxSize={{ base: "14px", md: "16px" }} />}
                iconSpacing={{ base: 1, md: 2 }}
                _hover={{ bg: "#244869" }}
                onClick={() => navigate("/create")}
              >
                <Text display={{ base: "none", sm: "inline" }}>Попробовать бесплатно</Text>
                <Text display={{ base: "inline", sm: "none" }}>Попробовать</Text>
              </Button>
            </HStack>
          </Flex>
                  ...
                  >
                    {submenuItems.map(({ label, hint, icon }, idx) => (
                      ...
                      >
                        ...
                        >
                          <Image src={icon} alt="" boxSize="24px" objectFit="contain" />
                        </Flex>
                        <VStack align="start" spacing={0}>
                          <HStack spacing={2}>
                            <Text fontSize={{ base: "14px", md: "16px" }} fontWeight="500" color="#12233F">
                              {label}
                            </Text>
                            {idx === 0 ? <Text color="#2D547B">→</Text> : null}
                          </HStack>
                          {hint ? (
                            <Text fontSize={{ base: "11px", md: "12px" }} color="#52525B">
                              {hint}
                            </Text>
                          ) : null}
                        </VStack>
            ...
            <HStack spacing={8} fontSize="16px" color="#12233F" fontWeight="500">
              ...
              <Box ref={desktopMenuContainerRef} position="relative">
                <HStack
                  ...
                  align="center"
                >
                  <Text w="148px" h="24px" display="flex" alignItems="center" lineHeight="24px">
                    Создать рассылку
                  </Text>
                  <Box boxSize="18px" display="flex" alignItems="center" justifyContent="center">
                    <Icon
                  ...
                  >
                    {submenuItems.map(({ label, hint, icon }, idx) => (
                      ...
                      >
                        ...
                        <VStack align="start" spacing={0}>
                          <HStack spacing={2}>
                            <Text fontSize="16px" fontWeight="500" color="#12233F">
                              {label}
                            </Text>
                            {idx === 0 ? <Text color="#2D547B">→</Text> : null}
                          </HStack>
      ...
      <Box mt={{ base: 10, lg: 6 }} pb={{ base: 16, lg: 20 }} position="relative" overflow="hidden">
        <Box
          position="absolute"
          inset={0}
        ...
        <Container maxW="1760px" px={{ base: 4, md: 8 }}>
          <VStack spacing={5} maxW={{ base: "328px", md: "736px", xl: "880px" }} mx="auto" textAlign="center" pt={{ base: 20, md: 24, xl: 40 }} position="relative" zIndex={1}>
            <Text
            ...
            <Text color="#52525B" fontSize={{ base: "18px", md: "20px" }} lineHeight={{ base: "28px", md: "30px" }}>
              Создавайте персонализированные рассылки через различные каналы.
              <br />
              Автоматизируйте коммуникацию и повышайте конверсию
            </Text>

            <HStack spacing={3.5} wrap="wrap" justify="center" maxW={{ base: "290px", md: "none" }}>
              {channelBadges.map((item) => (
                <Badge
                  ...
                  _hover={item.label === "e-mail" ? { bg: "#E2EBF3" } : undefined}
                  onClick={() => {
                ...
                >
                  <HStack spacing={2}>
                    <Image src={item.icon} alt="" boxSize="20px" objectFit="contain" />
          ...
          >
            {featureCards.map((card) => (
              <Box key={card.title} bg="white" border="1px solid #E2EBF3" borderRadius="24px" p={6}>
                <Image src={card.icon} alt="" boxSize="32px" objectFit="contain" />
                <Stack spacing={2} mt={6}>
                  <Text fontSize="20px" lineHeight="30px" fontWeight="600" color="#12233F">
                    {card.title}
                  </Text>
